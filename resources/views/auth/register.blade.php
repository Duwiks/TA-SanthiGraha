...
@section('content')
    <form method="POST" action="{{ url('/register') }}" class="space-y-4">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <button type="button" onclick="togglePassword()"
                    class="absolute inset-y-0 right-0 flex items-center pr-4 text-slate-400 hover:text-brand-600 focus:outline-none transition-colors">
        ...
        <button type="submit"
            class="w-full py-3 px-4 bg-gradient-to-r from-emerald-600 to-teal-500 text-white font-semibold rounded-xl shadow-lg shadow-brand-500/30 hover:shadow-brand-500/40 hover:-translate-y-0.5 transition-all duration-200 mt-4">
            Daftar Sekarang
        </button>
    </form>
    ...
    <div class="text-center mt-6 text-sm text-slate-500 font-medium">
        Sudah punya akun? <a href="{{ url('/login') }}"
            class="text-brand-600 hover:text-brand-700 hover:underline font-semibold transition-colors">Masuk di sini</a>
    </div>
