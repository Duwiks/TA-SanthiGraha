...
<form method="GET" action="{{ route('transactions.index') }}" class="mb-5 flex flex-wrap gap-3">
    ...
    <select name="type" class="px-4 py-2.5 rounded-xl border border-slate-200 text-sm bg-white focus:ring-2 focus:ring-emerald-400 outline-none">
    ...
    <select name="status" class="px-4 py-2.5 rounded-xl border border-slate-200 text-sm bg-white focus:ring-2 focus:ring-emerald-400 outline-none">
        <option value="">Semua Status</option>
        <option value="pending"  @selected(request('status') === 'pending')>Menunggu Persetujuan</option>
        <option value="approved" @selected(request('status') === 'approved')>Disetujui</option>
        <option value="rejected" @selected(request('status') === 'rejected')>Ditolak</option>
    </select>
    <button type="submit" class="px-5 py-2.5 rounded-xl bg-slate-700 text-white text-sm font-medium hover:bg-slate-800 transition-colors">Cari</button>
    @if(request()->anyFilled(['search', 'type', 'status']))
        <a href="{{ route('transactions.index') }}" class="px-5 py-2.5 rounded-xl bg-slate-100 text-slate-600 text-sm font-medium hover:bg-slate-200 transition-colors">Reset</a>
    @endif
</form>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-5 py-4 whitespace-nowrap">
                        <div class="font-medium text-slate-800">{{ \Carbon\Carbon::parse($trx->transaction_date)->format('d M Y') }}</div>
                        <div class="text-xs text-slate-400 mt-0.5">{{ \Carbon\Carbon::parse($trx->created_at)->format('H:i') }} WIB</div>
                    </td>
                    ...
                    <td class="px-5 py-4">
                        ...
                        @if($trx->description)
                            <div class="text-[12px] text-slate-400 mt-1 italic">{{ Str::limit($trx->description, 50) }}</div>
                        @endif
                    </td>
                    ...
                    <td class="px-5 py-4 whitespace-nowrap text-center">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            @else
                                <span class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-slate-50 text-slate-400 text-xs font-semibold border border-slate-100 cursor-not-allowed" title="Terkunci (Transaksi sudah valid/disetujui)">
                ...
                <tr>
                    <td colspan="6" class="px-6 py-16 text-center">
                        ...
                        <p class="text-sm text-slate-500 mt-1">Klik tombol "Ajukan Transaksi" untuk melaporkan pemasukan atau pengeluaran.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
...
<div class="mt-5 flex flex-wrap gap-3 text-xs text-slate-500">
    ...
    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-amber-400 inline-block"></span> Menunggu Persetujuan</span>
    <i class="ph ph-arrow-right text-slate-300"></i>
    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-emerald-400 inline-block"></span> Disetujui Admin</span>
    <span class="mx-1 text-slate-300">|</span>
    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-red-400 inline-block"></span> Ditolak (dapat diedit)</span>
</div>
