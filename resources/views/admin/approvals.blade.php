...
<div class="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    ...
    @if($transactions->total() > 0)
        <div class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 text-sm font-bold">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-5 py-4 whitespace-nowrap">
                        <div class="font-medium text-slate-800">{{ \Carbon\Carbon::parse($trx->created_at)->format('d M Y') }}</div>
                        <div class="text-xs text-slate-400 mt-0.5">{{ \Carbon\Carbon::parse($trx->created_at)->format('H:i') }} WIB</div>
                        <div class="text-xs text-slate-400 mt-1">Trx: {{ \Carbon\Carbon::parse($trx->transaction_date)->format('d/m/Y') }}</div>
                    </td>
    ...
    <div class="flex items-center gap-2 flex-wrap">
        <a href="{{ route('approvals.index') }}"
           class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold transition-all
            ...
            <span class="ml-1 text-[11px] font-bold px-1.5 py-0.5 rounded-full {{ !request('filter_status') ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500' }}">
        ...
        <a href="{{ route('approvals.index', ['filter_status' => 'approved']) }}"
           class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold transition-all
            ...
            <span class="ml-1 text-[11px] font-bold px-1.5 py-0.5 rounded-full {{ request('filter_status') === 'approved' ? 'bg-white/20 text-white' : 'bg-emerald-50 text-emerald-600' }}">
        ...
        <a href="{{ route('approvals.index', ['filter_status' => 'rejected']) }}"
           class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold transition-all
            ...
            <span class="ml-1 text-[11px] font-bold px-1.5 py-0.5 rounded-full {{ request('filter_status') === 'rejected' ? 'bg-white/20 text-white' : 'bg-red-50 text-red-600' }}">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-slate-600">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @php $isApproved = $trx->status === 'approved'; $lastRejection = $trx->rejections->last(); @endphp
                    <tr class="hover:bg-slate-50 transition-colors">

                        {{-- Tanggal Proses --}}
                        <td class="px-5 py-4 whitespace-nowrap">
                            <div class="font-medium text-slate-800">{{ \Carbon\Carbon::parse($trx->updated_at)->format('d M Y') }}</div>
                            <div class="text-xs text-slate-400 mt-0.5">{{ \Carbon\Carbon::parse($trx->updated_at)->format('H:i') }} WIB</div>
                            <div class="text-xs text-slate-400 mt-1">Trx: {{ \Carbon\Carbon::parse($trx->transaction_date)->format('d/m/Y') }}</div>
                        </td>
                        ...
                        <td class="px-5 py-4">
                            ...
                            <div class="text-xs text-slate-500 mt-0.5">{{ $trx->category->category_name ?? '-' }}</div>
                            @if($trx->description)
                                <div class="text-[12px] text-slate-400 mt-1 italic">{{ Str::limit($trx->description, 45) }}</div>
                            @endif
                        ...
                        <td class="px-5 py-4 whitespace-nowrap">
                            ...
                            <div class="flex items-center gap-1.5 mt-0.5">
                                ...
                                @if($trx->payment_method)
                                    <span class="text-[10px] uppercase font-medium text-slate-400">{{ $trx->payment_method }}</span>
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="px-5 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    @if($trx->approver)
                                        <div class="text-xs text-slate-400">{{ $isApproved ? '✓' : '✗' }} {{ $trx->approver->name }}</div>
                                    @endif
                                </div>
                            </div>
                        </td>
