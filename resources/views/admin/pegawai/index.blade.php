...
<form method="GET" action="{{ route('pegawai.index') }}" class="mb-5 flex flex-wrap gap-3">
    ...
    @if(request()->filled('search'))
        <a href="{{ route('pegawai.index') }}" class="px-5 py-2.5 rounded-xl bg-slate-100 text-slate-600 text-sm font-medium hover:bg-slate-200 transition-colors">Reset</a>
    @endif
</form>
        ...
        <table class="w-full text-left text-sm text-slate-600">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <div>
                                ...
                                @if(in_array($user->id, $activeUserIds))
                                    <div class="text-[11px] text-emerald-500 font-medium flex items-center gap-1 mt-0.5">
                                        <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse inline-block"></span> Aktif sekarang
                                    </div>
                                @endif
                            </div>
                        </div>
                    ...
                    {{-- Terdaftar --}}
                    <td class="px-5 py-4 whitespace-nowrap">
                        <div class="font-medium text-slate-700">{{ $user->created_at ? $user->created_at->format('d M Y') : '-' }}</div>
                        <div class="text-xs text-slate-400 mt-0.5">{{ $user->created_at ? $user->created_at->format('H:i') . ' WIB' : '' }}</div>
                    </td>
