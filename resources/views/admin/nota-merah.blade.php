...
<div class="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    ...
    <div class="flex gap-2 flex-wrap">
        @if($countMenungguPersetujuan > 0)
            <div class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 text-sm font-semibold">
                <i class="ph ph-clock text-base"></i> {{ $countMenungguPersetujuan }} Menunggu Persetujuan
            </div>
        @endif
        @if($countMenungguKonfirmasi > 0)
            <div class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-purple-50 border border-purple-200 text-purple-700 text-sm font-semibold">
        ...
        @if($countMenungguPersetujuan === 0 && $countMenungguKonfirmasi === 0)
            <div class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-semibold">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left text-sm text-slate-600">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors {{ in_array($nota->status, ['menunggu_persetujuan','menunggu_konfirmasi']) ? 'border-l-4 border-l-amber-400' : '' }}">
                    ...
                    <td class="px-5 py-4">
                        ...
                        @if($nota->description)
                            <div class="text-[12px] text-slate-400 mt-1 italic">{{ Str::limit($nota->description, 55) }}</div>
                        @endif
                    </td>
                    ...
                    <td class="px-5 py-4 text-center whitespace-nowrap">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold {{ $nota->status_color }}">
                        ...
                        @if($nota->status === 'ditolak' && $nota->rejection_reason)
                            <div class="text-[10px] text-red-400 italic mt-1.5 max-w-[130px] mx-auto leading-relaxed text-left">
                        ...
                        @if($nota->status === 'menunggu_konfirmasi' && $nota->realisasi_date)
                            <div class="text-[10px] text-purple-500 mt-1.5">
                                <i class="ph ph-calendar"></i> {{ \Carbon\Carbon::parse($nota->realisasi_date)->format('d M Y') }}
                            </div>
                        @endif
