...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    ...
    <div class="flex gap-3 flex-wrap">
        @if($countMenungguPersetujuan > 0)
            <div class="flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 text-sm font-semibold">
                <i class="ph ph-clock text-base"></i>
                {{ $countMenungguPersetujuan }} Menunggu Persetujuan
            </div>
        @endif
        @if($countMenungguKonfirmasi > 0)
            <div class="flex items-center gap-2 px-4 py-2 rounded-xl bg-purple-50 border border-purple-200 text-purple-700 text-sm font-semibold">
                <i class="ph ph-hourglass text-base"></i>
                {{ $countMenungguKonfirmasi }} Menunggu Konfirmasi
            </div>
        @endif
        @if($countMenungguPersetujuan === 0 && $countMenungguKonfirmasi === 0)
            <div class="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-semibold">
...
<form method="GET" action="{{ route('nota-merah.index') }}" class="mb-5 flex flex-wrap gap-3">
    <div class="relative flex-1 min-w-[200px]">
        ...
        <input type="text" name="search" value="{{ request('search') }}"
            ...
            class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 outline-none bg-white">
    </div>
    <select name="status" class="px-4 py-2.5 rounded-xl border border-slate-200 text-sm bg-white focus:ring-2 focus:ring-indigo-400 outline-none">
        ...
        <option value="selesai"              @selected(request('status') === 'selesai')>✔️ Selesai</option>
    </select>
    <button type="submit" class="px-5 py-2.5 rounded-xl bg-brand-500 text-white text-sm font-medium hover:bg-brand-600 transition-colors">Filter</button>
    @if(request()->anyFilled(['search','status']))
        <a href="{{ route('nota-merah.index') }}" class="px-5 py-2.5 rounded-xl bg-slate-100 text-slate-600 text-sm font-medium hover:bg-slate-200 transition-colors">Reset</a>
    @endif
</form>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left text-sm text-slate-600">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/70 transition-colors {{ in_array($nota->status, ['menunggu_persetujuan','menunggu_konfirmasi']) ? 'border-l-4 border-l-amber-400' : '' }}">

                    {{-- Tanggal & Pengaju --}}
                    <td class="px-5 py-4 whitespace-nowrap">
                        <div class="font-medium text-slate-800">{{ $nota->created_at->format('d M Y') }}</div>
                        <div class="text-xs text-slate-400">{{ $nota->created_at->format('H:i') }} WIB</div>
                        <div class="flex items-center gap-1.5 mt-2">
                            <div class="w-6 h-6 rounded-full bg-brand-500/10 text-brand-600 flex items-center justify-center font-bold text-[10px]">
                    ...
                    <td class="px-5 py-4">
                        ...
                        @if($nota->description)
                            <div class="text-[12px] text-slate-400 mt-1 italic max-w-[200px]">{{ Str::limit($nota->description, 60) }}</div>
                        @endif
                    </td>

                    {{-- Nominal --}}
                    <td class="px-5 py-4 whitespace-nowrap">
                        <span class="font-bold text-red-600 text-base">Rp {{ number_format($nota->amount, 0, ',', '.') }}</span>
                                    ...
                                    @else
                                        <a href="{{ asset('storage/' . $nota->realisasi_photo) }}" target="_blank">
                                            <img src="{{ asset('storage/' . $nota->realisasi_photo) }}" alt="Realisasi"
                                                 class="h-14 w-20 object-cover rounded-lg border border-purple-200 hover:opacity-80 transition-opacity">
                                        </a>
                                    @endif
                                </div>
                            @endif
                    ...
                    <td class="px-5 py-4 text-center whitespace-nowrap">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold {{ $nota->status_color }}">
                        ...
                        @if($nota->status === 'ditolak' && $nota->rejection_reason)
                            <div class="text-[10px] text-red-400 italic mt-1.5 max-w-[130px] text-left mx-auto leading-relaxed">
                    ...
                    <td class="px-5 py-4 text-center whitespace-nowrap">
                        <div class="flex flex-col items-center gap-2">
                            ...
                            {{-- Tahap 2: Menunggu Konfirmasi (bukti realisasi sudah diupload) --}}
                            @elseif($nota->status === 'menunggu_konfirmasi')
                                <div class="text-xs text-purple-600 font-semibold mb-1 flex items-center gap-1">
                                    <i class="ph ph-receipt"></i>
                                    Tgl: {{ $nota->realisasi_date ? \Carbon\Carbon::parse($nota->realisasi_date)->format('d M Y') : '-' }}
                                </div>
                                <form action="{{ route('nota-merah.confirm', $nota->id) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-purple-500 text-white font-semibold text-xs hover:bg-purple-600 hover:shadow-lg hover:shadow-purple-500/20 transition-all">
                            ...
                            {{-- Sudah Disetujui, menunggu pegawai upload realisasi --}}
                            @elseif($nota->status === 'disetujui')
                                ...
                                <span class="text-[10px] text-slate-400">Pegawai belum upload bukti</span>
